 ...
 * }>} MessageMap
 */
/** @type {WeakMap<ChildProcess, MessageMap>} */
...
function getBestWorkerPath(prerequisites) {
  ...
  const version = Number.parseInt(eslintMajorVersion, 10);

  if (version === 8) {
    if (
      prerequisites.eslintConfig.endsWith('eslint.config.js') ||
      prerequisites.eslintConfig.endsWith('eslint.config.cjs') ||
      prerequisites.eslintConfig.endsWith('eslint.config.mjs') ||
      prerequisites.eslintConfig.endsWith('eslint.config.ts') ||
      prerequisites.eslintConfig.endsWith('eslint.config.cts') ||
      prerequisites.eslintConfig.endsWith('eslint.config.mts')
    ) {
    ...
    Workers.set(workerKey, (() => {
      ...
      worker.on('message', (message) => {
        ...
        if (messages.has(message.key) === false) {
          Console.error(`[${message.key}] Handlers not found`);
          return;
        }
        ...
        if (message.reject != null) {
          request.reject(message.reject);
          return;
        }

        if (message.resolve != null) {
          request.resolve(message.resolve);
        }
      });
